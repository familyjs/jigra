...
open class JigraUrlRequest: NSObject, URLSessionTaskDelegate {
    public var request: URLRequest
    public var headers: [String: String]

    public enum JigraUrlRequestError: Error {
    ...
    public func getRequestDataAsJson(_ data: JSValue) throws -> Data? {
        ...
        if JSONSerialization.isValidJSONObject(data) {
            ...
            throw JigraUrlRequest.JigraUrlRequestError.serializationError("[ data ] argument for request of content-type [ application/json ] must be serializable to JSON")
        }
    }
    ...
    public func getRequestDataAsFormUrlEncoded(_ data: JSValue) throws -> Data? {
        ...
        guard let obj = data as? JSObject else {
            ...
            throw JigraUrlRequestError.serializationError("[ data ] argument for request with content-type [ multipart/form-data ] may only be a plain javascript object")
        }
    ...
    public func getRequestDataAsMultipartFormData(_ data: JSValue) throws -> Data {
        guard let obj = data as? JSObject else {
            // Throw, other data types explicitly not supported.
            throw JigraUrlRequestError.serializationError("[ data ] argument for request with content-type [ application/x-www-form-urlencoded ] may only be a plain javascript object")
        }
    ...
    public func getRequestDataAsString(_ data: JSValue) throws -> Data {
        guard let stringData = data as? String else {
            throw JigraUrlRequestError.serializationError("[ data ] argument could not be parsed as string")
        }
    ...
    public func getRequestData(_ body: JSValue, _ contentType: String) throws -> Data? {
        ...
        if let strVal = try? getRequestDataAsString(body) {
            ...
            throw JigraUrlRequestError.serializationError("[ data ] argument could not be parsed for content type [ \(contentType) ]")
        }
    }
    ...
    open func urlSession(_ session: URLSession, task: URLSessionTask, willPerformHTTPRedirection response: HTTPURLResponse,
                         newRequest request: URLRequest, completionHandler: @escaping (URLRequest?) -> Void) {
        completionHandler(nil)
    }

    open func getUrlSession(_ call: JIGPluginCall) -> URLSession {
