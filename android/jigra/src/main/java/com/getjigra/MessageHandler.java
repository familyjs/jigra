...
public class MessageHandler {
    ...
    public MessageHandler(Bridge bridge, WebView webView, PluginManager cordovaPluginManager) {
        ...
        if (WebViewFeature.isFeatureSupported(WebViewFeature.WEB_MESSAGE_LISTENER) && !bridge.getConfig().isUsingLegacyBridge()) {
            WebViewCompat.WebMessageListener jigListener = (view, message, sourceOrigin, isMainFrame, replyProxy) -> {
                if (isMainFrame) {
            ...
            try {
                WebViewCompat.addWebMessageListener(webView, "androidBridge", bridge.getAllowedOriginRules(), jigListener);
            } catch (Exception ex) {
    ...
    public void postMessage(String jsonStr) {
        try {
            ...
            if (isCordovaPlugin) {
                ...
                Logger.verbose(
                    Logger.tags("Plugin"),
                    "To native (Jigra plugin): callbackId: " + callbackId + ", pluginId: " + pluginId + ", methodName: " + methodName
                );
