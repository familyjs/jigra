  ...
  return new Promise((resolve, reject) => {
    const p = spawn(cmd, {
      ...
      env: {
        ...
        JIGRA_ROOT_DIR: config.app.rootDir,
        JIGRA_WEB_DIR: config.app.webDirAbs,
        JIGRA_CONFIG: JSON.stringify(config.app.extConfig),
        JIGRA_PLATFORM_NAME: platformName,
        ...process.env,
      },
    });
    p.on('close', () => {
...
export async function getCoreVersion(config: Config): Promise<string> {
  return getJigraPackageVersion(config, 'core');
}

export async function getCLIVersion(config: Config): Promise<string> {
  return getJigraPackageVersion(config, 'cli');
}
