...
@objc(JIGNotificationRouter) public class NotificationRouter: NSObject, UNUserNotificationCenterDelegate {
    ...
    public weak var pushNotificationHandler: NotificationHandlerProtocol? {
        didSet {
            if pushNotificationHandler != nil, oldValue != nil {
                JIGLog.print("Push notification handler overriding previous instance: \(String(describing: type(of: oldValue)))")
            }
        }
    }
    ...
    public weak var localNotificationHandler: NotificationHandlerProtocol? {
        didSet {
            if localNotificationHandler != nil, oldValue != nil {
                JIGLog.print("Local notification handler overriding previous instance: \(String(describing: type(of: oldValue)))")
            }
        }
    }
