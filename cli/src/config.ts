...
const debug = Debug('jigra:config');

export const CONFIG_FILE_NAME_TS = 'jigra.config.ts';
export const CONFIG_FILE_NAME_JS = 'jigra.config.js';
export const CONFIG_FILE_NAME_JSON = 'jigra.config.json';
...
async function determineAndroidStudioPath(os: OS): Promise<string> {
  if (process.env.JIGRA_ANDROID_STUDIO_PATH) {
    return process.env.JIGRA_ANDROID_STUDIO_PATH;
  }
