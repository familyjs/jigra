...
@JigraPlugin
public class JigraCookies extends Plugin {

    JigraCookieManager cookieManager;

    @Override
    public void load() {
        this.bridge.getWebView().addJavascriptInterface(this, "JigraCookiesAndroidInterface");
        super.load();
    }

    @JavascriptInterface
    public boolean isEnabled() {
        PluginConfig pluginConfig = getBridge().getConfig().getPluginConfiguration("JigraCookies");
        boolean isEnabled = pluginConfig.getBoolean("enabled", false);
        if (isEnabled) {
            this.cookieManager = new JigraCookieManager(null, java.net.CookiePolicy.ACCEPT_ALL, this.bridge);
    ...
    /**
     * Helper function for getting the serverUrl from the Jigra Config. Returns an empty
     * string if it is invalid and will auto-reject through {@code call}
     * @param call the {@code PluginCall} context
     * @return the string of the server specified in the Jigra config
     */
