...
public class JigConfig {
    ...
    /**
     * Constructs a Jigra Configuration from config.json file within the app assets.
     *
     * @param context The context.
     * @param path A path relative to the root assets directory.
     * @return A loaded config file, if successful.
     */
    public static JigConfig loadFromAssets(Context context, String path) {
        JigConfig config = new JigConfig();
    ...
    /**
     * Constructs a Jigra Configuration from config.json file within the app file-space.
     *
     * @param context The context.
     * @param path A path relative to the root of the app file-space.
     * @return A loaded config file, if successful.
     */
    public static JigConfig loadFromFile(Context context, String path) {
        JigConfig config = new JigConfig();
    ...
    private void loadConfigFromAssets(AssetManager assetManager, String path) {
        ...
        try {
            String jsonString = readFileFromAssets(assetManager, path + "jigra.config.json");
    ...
    private void loadConfigFromFile(String path) {
        ...
        try {
            File configFile = new File(path + "jigra.config.json");
            String jsonString = FileUtils.readFileFromDisk(configFile);
            configJSON = new JSONObject(jsonString);
        } catch (JSONException ex) {
            Logger.error("Unable to parse jigra.config.json. Make sure it's valid json", ex);
        } catch (IOException ex) {
            Logger.error("Unable to load jigra.config.json.", ex);
        }
    }
