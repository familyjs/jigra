#ifndef JIGInstanceDescriptor_h
#define JIGInstanceDescriptor_h

@import UIKit;
@import Cordova;

typedef NS_ENUM(NSInteger, JIGInstanceType) {
    JIGInstanceTypeFixed NS_SWIFT_NAME(fixed),
    JIGInstanceTypeVariable NS_SWIFT_NAME(variable)
} NS_SWIFT_NAME(InstanceType);

typedef NS_OPTIONS(NSUInteger, JIGInstanceWarning) {
    JIGInstanceWarningMissingAppDir       NS_SWIFT_NAME(missingAppDir)      = 1 << 0,
    JIGInstanceWarningMissingFile         NS_SWIFT_NAME(missingFile)        = 1 << 1,
    JIGInstanceWarningInvalidFile         NS_SWIFT_NAME(invalidFile)        = 1 << 2,
    JIGInstanceWarningMissingCordovaFile  NS_SWIFT_NAME(missingCordovaFile) = 1 << 3,
    JIGInstanceWarningInvalidCordovaFile  NS_SWIFT_NAME(invalidCordovaFile) = 1 << 4
} NS_SWIFT_NAME(InstanceWarning);

typedef NS_OPTIONS(NSUInteger, JIGInstanceLoggingBehavior) {
    JIGInstanceLoggingBehaviorNone          NS_SWIFT_NAME(none)         = 1 << 0,
    JIGInstanceLoggingBehaviorDebug         NS_SWIFT_NAME(debug)        = 1 << 1,
    JIGInstanceLoggingBehaviorProduction    NS_SWIFT_NAME(production)   = 1 << 2,
} NS_SWIFT_NAME(InstanceLoggingBehavior);

extern NSString * _Nonnull const JIGInstanceDescriptorDefaultScheme NS_SWIFT_UNAVAILABLE("Use InstanceDescriptorDefaults");
extern NSString * _Nonnull const JIGInstanceDescriptorDefaultHostname NS_SWIFT_UNAVAILABLE("Use InstanceDescriptorDefaults");

NS_SWIFT_NAME(InstanceDescriptor)
@interface JIGInstanceDescriptor : NSObject
/**
...
/**
 ...
 @discussion Defaults to @c jigra. Set by @c server.iosScheme in the configuration file.
 */
 ...
 */
@property (nonatomic, assign) JIGInstanceLoggingBehavior loggingBehavior;
/**
...
/**
 @brief Whether or not the Jigra runtime will set itself as the @c UNUserNotificationCenter delegate.
...
/**
 @brief The base file URL from which Jigra will load resources
...
/**
 @brief The path (relative to @c appLocation) which Jigra will use for the inital URL at launch.
 @discussion Defaults to nil, in which case Jigra will attempt to load @c index.html.
 */
@property (nonatomic, copy, nullable) NSString *appStartPath;
/**
 @brief Whether or not the Jigra WebView will limit the navigation to @c WKAppBoundDomains listed in the Info.plist.
 ...
 */
@property (nonatomic, assign) JIGInstanceWarning warnings;
/**
 @brief The type of instance.
 */
@property (nonatomic, readonly) JIGInstanceType instanceType;
/**
...
/**
 @brief Initialize the descriptor with the default environment. This assumes that the application was built with the help of the Jigra CLI and that that the web app is located inside the application bundle at @c public/.
 */
...
/**
 ...
 @param configURL The location of the Jigra configuration file.
