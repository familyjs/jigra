...
public enum InstanceDescriptorDefaults {
    public static let scheme = "jigra"
...
internal extension InstanceDescriptor {
    ...
    @objc func _parseConfiguration(at jigraURL: URL?, cordovaConfiguration cordovaURL: URL?) {
        ...
        // parse the jigra configuration
        var config: JSObject?
        if let jigraURL = jigraURL,
           FileManager.default.fileExists(atPath: jigraURL.path, isDirectory: &isDirectory),
           isDirectory.boolValue == false {
            do {
                let contents = try Data(contentsOf: jigraURL)
        ...
        if let config = config {
            ...
            if let colorString = (config[keyPath: "ios.backgroundColor"] as? String) ?? (config[keyPath: "backgroundColor"] as? String),
               let color = UIColor.jigra.color(fromHex: colorString) {
