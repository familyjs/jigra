...
describe.each([false, true])('Add: Android (monoRepoLike: %p)', (monoRepoLike) => {
  ...
  it('Should rename package', async () => {
    expect(await FS.exists('android/app/src/main/java/com/getjigra/cli/test/MainActivity.java')).toBe(true);
  });

  it('Should rename package in main activity', async () => {
    const activityContent = await FS.read('android/app/src/main/java/com/getjigra/cli/test/MainActivity.java');
  ...
  // Other test ideas:
  // should install/copy pre-existing cordova/jigra plugins in package.json
});
