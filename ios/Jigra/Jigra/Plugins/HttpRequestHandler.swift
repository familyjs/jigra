...
open class HttpRequestHandler {
    open class JigraHttpRequestBuilder {
        public var url: URL?
        public var method: String?
        public var params: [String: String]?
        open var request: JigraUrlRequest?
        ...
        public func setUrl(_ urlString: String) throws -> JigraHttpRequestBuilder {
        ...
        public func setMethod(_ method: String) -> JigraHttpRequestBuilder {
            self.method = method
            return self
        }

        public func setUrlParams(_ params: [String: Any]) -> JigraHttpRequestBuilder {
            if params.count != 0 {
        ...
        open func openConnection() -> JigraHttpRequestBuilder {
            request = JigraUrlRequest(url!, method: method!)
            return self
        }

        public func build() -> JigraUrlRequest {
            return request!
        }
    }
        ...
        if let cookies = headers["Set-Cookie"] as? String {
            for cookie in cookies.components(separatedBy: ",") {
                ...
                if domainComponents.count > 1 {
                    JigraCookieManager(config).setCookie(
                        domainComponents[1].components(separatedBy: ";")[0],
                        cookie
                    )
                } else {
                    JigraCookieManager(config).setCookie("", cookie)
                }
            }
        }
        JigraCookieManager(config).syncCookiesToWebView()
    }
    ...
    public static func request(_ call: JIGPluginCall, _ httpMethod: String?, _ config: InstanceConfiguration?) throws {
        ...
        let request = try JigraHttpRequestBuilder()
            .setUrl(urlString)
            .setMethod(method)
            .setUrlParams(params)
            .openConnection()
