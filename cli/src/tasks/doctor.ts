...
export async function doctorCommand(config: Config, selectedPlatformName: string): Promise<void> {
  output.write(`${_e('💊', '')}   ${c.strong('Jigra Doctor')}  ${_e('💊', '')} \n\n`);
...
export async function doctorCore(config: Config): Promise<void> {
  const [cliVersion, coreVersion, androidVersion, iosVersion] = await Promise.all([
    getCommandOutput('npm', ['info', '@jigra/cli', 'version']),
    getCommandOutput('npm', ['info', '@jigra/core', 'version']),
    getCommandOutput('npm', ['info', '@jigra/android', 'version']),
    getCommandOutput('npm', ['info', '@jigra/ios', 'version']),
  ]);

  output.write(
    `${c.strong('Latest Dependencies:')}\n\n` +
      `  @jigra/cli: ${c.weak(cliVersion ?? 'unknown')}\n` +
      `  @jigra/core: ${c.weak(coreVersion ?? 'unknown')}\n` +
      `  @jigra/android: ${c.weak(androidVersion ?? 'unknown')}\n` +
      `  @jigra/ios: ${c.weak(iosVersion ?? 'unknown')}\n\n` +
