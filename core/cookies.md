...
The Jigra Cookies API provides native cookie support via patching `document.cookie` to use native libraries. It also provides methods for modifying cookies at a specific url. This plugin is bundled with `@jigra/core`.
...
```typescript
import { JigraCookies } from '@jigra/core';
...
const setJigraCookie = async () => {
  await JigraCookies.setCookie({
...
const deleteCookie = async () => {
  await JigraCookies.deleteCookie({
...
const clearCookiesOnUrl = async () => {
  await JigraCookies.clearCookies({
    url: 'https://example.com',
  });
};

const clearAllCookies = async () => {
  await JigraCookies.clearAllCookies();
};
```
