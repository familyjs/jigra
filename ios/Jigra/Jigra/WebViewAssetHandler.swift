...
internal class WebViewAssetHandler: NSObject, WKURLSchemeHandler {
    ...
    func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
        ...
        if url.path.starts(with: JigraBridge.httpInterceptorStartIdentifier) {
            handleJigraHttpRequest(urlSchemeTask, localUrl, false)
            return
        }
        ...
        let task = urlSession.dataTask(with: urlRequest) { (data, response, error) in
            ...
            if let response = response as? HTTPURLResponse {
                ...
                if let mergedHeaders = existingHeaders
                    .merging(newHeaders, uniquingKeysWith: { (current, _) in current }) as? [String: String] {
