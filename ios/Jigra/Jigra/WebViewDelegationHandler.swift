...
internal class WebViewDelegationHandler: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    ...
    public func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        NotificationCenter.default.post(name: .jigraDecidePolicyForNavigationAction, object: navigationAction)
        ...
        do {
            if let dataFromString = prompt.data(using: .utf8, allowLossyConversion: false) {
                if let payload = try JSONSerialization.jsonObject(with: dataFromString, options: .fragmentsAllowed) as? [String: AnyObject] {
                    ...
                    if type == "JigraCookies.get" {
                        completionHandler(JigraCookieManager(bridge!.config).getCookies())
                        // Don't present prompt
                        return
                    } else if type == "JigraCookies.set" {
                        // swiftlint:disable force_cast
                        let action = payload["action"] as! String
                        let domain = payload["domain"] as! String
                        JigraCookieManager(bridge!.config).setCookie(domain, action)
                        completionHandler("")
                        // swiftlint:enable force_cast
                        // Don't present prompt
                        return
                    } else if type == "JigraCookies.isEnabled" {
                        let pluginConfig = bridge!.config.getPluginConfig("JigraCookies")
                        completionHandler(String(pluginConfig.getBoolean("enabled", false)))
                        // Don't present prompt
                        return
                    } else if type == "JigraHttp" {
                        let pluginConfig = bridge!.config.getPluginConfig("JigraHttp")
                        completionHandler(String(pluginConfig.getBoolean("enabled", false)))
                        // Don't present prompt
                        return
                    }
