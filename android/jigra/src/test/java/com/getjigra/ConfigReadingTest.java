...
public class ConfigReadingTest {
    ...
    public void bad() {
        try {
            when(assetManager.open("jigra.config.json")).thenReturn(getTestInputStream(BAD_TEST));
    ...
    public void flat() {
        try {
            when(assetManager.open("jigra.config.json")).thenReturn(getTestInputStream(FLAT_TEST));
    ...
    public void hierarchy() {
        try {
            when(assetManager.open("jigra.config.json")).thenReturn(getTestInputStream(HIERARCHY_TEST));
    ...
    public void nonJSON() {
        try {
            final String errText = "Unable to parse jigra.config.json. Make sure it's valid json";

            when(assetManager.open("jigra.config.json")).thenReturn(getTestInputStream(NONJSON_TEST));
    ...
    public void server() {
        try {
            when(assetManager.open("jigra.config.json")).thenReturn(getTestInputStream(SERVER_TEST));
