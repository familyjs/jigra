...
import static com.getjigra.plugin.util.HttpRequestHandler.isDomainExcludedFromSSL;
...
public class WebViewLocalServer {
    ...
    private static final String jigraFileStart = Bridge.JIGRA_FILE_START;
    private static final String jigraContentStart = Bridge.JIGRA_CONTENT_START;
    ...
    public WebResourceResponse shouldInterceptRequest(WebResourceRequest request) {
        ...
        if (null != loadingUrl.getPath() && loadingUrl.getPath().startsWith(Bridge.JIGRA_HTTP_INTERCEPTOR_START)) {
            Logger.debug("Handling JigraHttp request: " + loadingUrl);
            try {
                return handleJigraHttpRequest(request);
            } catch (Exception e) {
    ...
    private boolean isLocalFile(Uri uri) {
        String path = uri.getPath();
        return path.startsWith(jigraContentStart) || path.startsWith(jigraFileStart);
    }
    ...
    private WebResourceResponse handleJigraHttpRequest(WebResourceRequest request) throws IOException {
        String urlString = request.getUrl().getQueryParameter(Bridge.JIGRA_HTTP_INTERCEPTOR_URL_PARAM);
    ...
    /**
     * Prepends an {@code InputStream} with the JavaScript required by Jigra.
        ...
        PathHandler handler = new PathHandler() {
            ...
            public InputStream handle(Uri url) {
                ...
                try {
                    ...
                    } else if (path.startsWith(jigraFileStart)) {
